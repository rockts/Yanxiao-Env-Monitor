...
if __name__ == "__main__":
    ...
    parser.add_argument("--host", default=DEFAULT_HOST, help=f"监听的主机地址 (默认: {DEFAULT_HOST})")
    parser.add_argument("--port", type=int, default=DEFAULT_PORT, help=f"监听的端口 (默认: {DEFAULT_PORT})")
    args = parser.parse_args()
    
    # 创建并启动服务器
    broker = MQTTProxyServer(host=args.host, port=args.port)
    ...
    try:
        broker.start()
        logger.info(f"MQTT代理已启动，监听于 {args.host}:{args.port}")
