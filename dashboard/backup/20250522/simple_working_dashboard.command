...
# 显示标题
echo -e "${BLUE}=======================================================${NC}"
echo -e "${GREEN}     烟铺小学智慧校园环境监测系统 - 简单工作版      ${NC}"
echo -e "${BLUE}=======================================================${NC}"
echo -e "${YELLOW}     支持传感器数据显示和实时视频监控      ${NC}"
echo -e "${BLUE}=======================================================${NC}"
echo
...
$PYTHON_CMD -c "import tkinter; import paho.mqtt.client" 2>/dev/null
if [ $? -ne 0 ]; then
    echo -e "${RED}缺少必要的Python库.${NC}"
    echo -e "${YELLOW}正在安装必要的Python库...${NC}"
    $PYTHON_CMD -m pip install paho-mqtt
