#!/bin/bash
# 智慧校园环境监测系统 - 一键启动脚本（简化版仪表盘 + 模拟MQTT数据）
...
echo -e "${BLUE}============================================${NC}"
echo -e "${GREEN} 智慧校园环境监测系统（带模拟MQTT服务） ${NC}"
echo -e "${BLUE}============================================${NC}"
...
def send_data_periodically(client):
    ...
    while not stop_flag:
        ...
        for topic, value in data_values.items():
            ...
            client.publish(topic_str, str(value))
            print(f"发送: {topic_str} = {value}")
        
        # 特别确保AQI数据被发送
        client.publish("siot/aqi", str(data_values["aqi"]))
        print(f"再次发送AQI数据: siot/aqi = {data_values['aqi']}")
...
def main():
    ...
    # 创建客户端
    client = mqtt.Client("mqtt_simulator")
