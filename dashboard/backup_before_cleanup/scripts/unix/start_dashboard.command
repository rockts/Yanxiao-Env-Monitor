#!/bin/bash
# 修复版本的智慧校园仪表盘 macOS 启动脚本 - 运行新版本代码

# 获取脚本所在目录的绝对路径
SCRIPT_DIR="$( cd "$( dirname "${BASH_SOURCE[0]}" )" &> /dev/null && pwd )"
cd "$SCRIPT_DIR"

# 设置颜色用于输出
GREEN='\033[0;32m'
YELLOW='\033[1;33m'
BLUE='\033[0;34m'
RED='\033[0;31m'
NC='\033[0m' # 无颜色

echo -e "${BLUE}============================================${NC}"
echo -e "${GREEN}   烟铺小学智慧校园仪表盘启动程序 (重构版)   ${NC}"
echo -e "${BLUE}============================================${NC}"
echo

# 确定Python解释器
if command -v python3 &> /dev/null; then
    PYTHON_CMD="python3"
    echo -e "${GREEN}使用Python解释器: $PYTHON_CMD$(python3 --version)${NC}"
else
    echo -e "${RED}错误: 未找到Python 3.${NC}"
    echo -e "${YELLOW}请安装Python 3后再试.${NC}"
    echo
    echo "按任意键退出..."
    read -n 1
    exit 1
fi

# 检查必要的包
echo -e "${BLUE}检查必要的Python包...${NC}"
PACKAGES=("tkinter" "paho-mqtt" "pillow" "matplotlib" "requests")

for package in "${PACKAGES[@]}"; do
    if [ "$package" = "tkinter" ]; then
        # tkinter特殊处理
        if ! python3 -c "import tkinter" &>/dev/null; then
            echo -e "${RED}错误: tkinter未安装或无法导入.${NC}"
            echo -e "${YELLOW}tkinter通常随Python一起安装，但某些系统可能需要单独安装.${NC}"
            echo "在macOS上，可以尝试重新安装Python或使用brew install python-tk"
            echo
            echo "按任意键退出..."
            read -n 1
            exit 1
        else
            echo -e "${GREEN}✓ tkinter 已安装${NC}"
        fi
    else
        if ! python3 -c "import $package" &>/dev/null; then
            echo -e "${YELLOW}正在安装 $package...${NC}"
            python3 -m pip install $package
        else
            echo -e "${GREEN}✓ $package 已安装${NC}"
        fi
    fi
done

echo
echo -e "${BLUE}启动仪表盘应用(新版)...${NC}"
cd "$SCRIPT_DIR"

# 使用新的main_dashboard.py入口文件
$PYTHON_CMD src/main_dashboard.py

echo
echo -e "${GREEN}程序已退出.${NC}"
echo "按任意键关闭此窗口..."
read -n 1
