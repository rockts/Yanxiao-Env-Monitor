...
def main():
    ...
    # 首先尝试使用mosquitto_pub验证是否已有MQTT代理在运行
    try:
        # 创建客户端来测试连接
        client = mqtt.Client(mqtt.CallbackAPIVersion.VERSION2, client_id="test-client-" + str(time.time()))
        client.connect(args.host, args.port, keepalive=1)
        client.disconnect()
        print(f"检测到已有MQTT代理在运行于 {args.host}:{args.port}")
        print("将使用该代理而不启动新的服务")
        
        # 创建转发客户端
        client = mqtt.Client(mqtt.CallbackAPIVersion.VERSION2, client_id="mqtt-relay-" + str(time.time()))
        client.on_connect = on_connect
        client.on_disconnect = on_disconnect
        client.on_message = on_message
        
        print(f"启动MQTT中继连接到 {args.host}:{args.port}")
        print("按Ctrl+C停止")
