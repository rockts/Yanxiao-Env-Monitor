...
# 显示标题
echo -e "${BLUE}=======================================================${NC}"
echo -e "${GREEN}  烟铺小学智慧校园环境监测系统 - 修复版启动脚本   ${NC}"
echo -e "${BLUE}=======================================================${NC}"
...
# 检查修复版本的仪表盘启动脚本
FIXED_DASHBOARD="$SCRIPT_DIR/fixed_dashboard.py"
if [ ! -f "$FIXED_DASHBOARD" ]; then
    echo -e "${RED}错误: 找不到修复版仪表盘脚本.${NC}"
    echo -e "${YELLOW}请确保 fixed_dashboard.py 在当前目录中.${NC}"
    read -p "按回车键退出..."
    exit 1
fi
...
if [[ "$OSTYPE" == "darwin"* ]]; then
    ...
    tell application "Terminal"
        do script "cd '$SCRIPT_DIR' && '$PYTHON_CMD' '$FIXED_DASHBOARD'"
        ...
        set the title of window 1 to "烟铺小学智慧校园环境监测系统 - 修复版"
        activate
    end tell
EOF
    
    echo -e "${GREEN}已在新终端窗口中启动修复版仪表盘${NC}"
    echo -e "${YELLOW}请查看新打开的终端窗口${NC}"
else
    # 非macOS系统
    echo -e "${YELLOW}非macOS系统，直接启动...${NC}"
    "$PYTHON_CMD" "$FIXED_DASHBOARD"
fi

echo
echo "仪表盘已在新窗口中启动，此窗口可以关闭"
echo "如果新窗口没有打开，请尝试手动运行: python3 fixed_dashboard.py"
read -p "按回车键关闭此窗口..."
