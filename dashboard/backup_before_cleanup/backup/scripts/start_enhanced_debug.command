#!/bin/bash
# 增强版调试启动脚本 - 同时启动MQTT代理、测试数据发送器和仪表盘
# 作者：GitHub Copilot
...
echo -e "${BLUE}============================================${NC}"
echo -e "${GREEN}   烟铺小学智慧校园仪表盘 - 增强调试模式   ${NC}"
echo -e "${BLUE}============================================${NC}"
echo -e "${YELLOW}此脚本将启动本地MQTT代理、测试数据发送器和仪表盘${NC}"
echo
...
# 检查MQTT代理所需的库
$PYTHON_CMD -c "import hbmqtt" 2>/dev/null || { 
    echo -e "${YELLOW}安装MQTT代理所需的hbmqtt库...${NC}"
    $PYTHON_CMD -m pip install hbmqtt
}
...
# 启动本地MQTT代理服务器
echo -e "${GREEN}启动本地MQTT代理服务器...${NC}"
$PYTHON_CMD simple_mqtt_broker.py --host $MQTT_HOST --port $MQTT_PORT &
BROKER_PID=$!
echo "MQTT代理服务器PID: $BROKER_PID"
